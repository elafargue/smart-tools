...
function analyse_tc(atr) {
	...
	if (counter == 1)
	{
		result += "Extra guard time: " + value;
		 if (value == 255) result += " (special value)";
	}
...
function analyse_td(atr) {
	...
	if (Y & 0x1) result += analyse_ta(atr);

	if (atr.length ==0) return result;
	if (Y & 0x2) result += analyse_tb(atr);

	if (atr.length ==0) return result;
	if (Y & 0x4) result += analyse_tc(atr);

	if (atr.length ==0) return result;
	if (Y & 0x8) result += analyse_td(atr);
...
function analyse_historical_bytes(atr) {
	...
	switch (hb_category)
	{
		...
		case 0x80 :
			result += " (compact TLV data object) <br>";
			result += "<ul>";
			while (atr.length) result += compact_tlv(atr);
			result += "</ul>";
			break;
...
function compact_tlv(atr)
{
	...
	switch (tag)
	{
		...
		case 0x4 :
			...
			if (len == 0xF) result += aid_parse(atr);
			break;
		case 0x5:
...
function cs_parse(csbyte)
{
	...
	result = "";
	if (cs & 0x80) result += "        - Application selection: by full DF name<br>";
	if (cs & 0x40) result += "        - Application selection: by partial DF name<br>";
	if (cs & 0x20) result += "        - BER-TLV data objects available in EF.DIR<br>";
	if (cs & 0x10) result += "        - BER-TLV data objects available in EF.ATR<br>";

	v = cs & 0xE;
	result += "        - EF.DIR and EF.ATR access services: ";
	switch (v)
	{
		case 8:  result += "by READ BINARY command<br>"; break;
		case 0:  result += "by GET RECORD(s) command<br>"; break;
		case 4:  result += "by GET DATA command<br>"; break;
		default: result += "reserved for future use";
	}
